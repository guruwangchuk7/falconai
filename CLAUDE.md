...
A follow-up full end-to-end run (`npm run dev:livekit` with a real participant, real webhook delivery, real Deepgram/Postgres/Redis) is in progress — see `ROADMAP.md` for the current state, including the durationMs bigint bug found and fixed above, and an open question (paused mid-investigation) about whether real transcribed text is reaching Deepgram at all: transcript events were landing in Redis with correct structure, sequencing, and participant identity, but every one observed so far had empty `text` and `confidence: 0`, which could mean silence/mic issue on the test client rather than a pipeline bug — unconfirmed either way.
