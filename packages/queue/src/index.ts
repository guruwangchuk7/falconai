...
export interface MeetingExtractJob { workspaceId: string; meetingId: string }
...
let _meetingExtract: Queue<MeetingExtractJob> | undefined;
...
export const meetingExtractQueue = (): Queue<MeetingExtractJob> =>
  (_meetingExtract ??= new Queue<MeetingExtractJob>('meeting-extract', { connection: conn() }));
...
/** Stable, coalescing meeting-extract job id: a finalized meeting is immutable, so one extraction
 *  per meeting is enough — no version/hash/day component (contrast mineJobId). */
export function meetingExtractJobId(workspaceId: string, meetingId: string): string {
  return `meeting-extract:${workspaceId}:${meetingId}`;
}
