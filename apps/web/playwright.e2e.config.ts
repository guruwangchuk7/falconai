 ...
 * and tears it down. Scoped to falcon.e2e.spec.ts so it doesn't also pick up the smoke suite.
 * Run with `pnpm --filter @falcon/web e2e:auth` (needs Docker + `playwright install chromium`).
 */
export default defineConfig({
  testDir: './e2e',
  testMatch: /falcon\.e2e\.spec\.ts/,
  timeout: 60_000,
  fullyParallel: false,
  workers: 1,
